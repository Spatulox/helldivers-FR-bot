        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!message.author.bot) {
                ...
                try {
                    ...
                    const auth = yield simplediscordbot_1.GuildManager.user.findInGuild(this.guildId, message.author.id);
                    if (auth == null) {
                        return;
                    }
                    let title = this.isStaff(auth) ? sanction_1.SanctionTitle.EXCLUSION_7D : sanction_1.SanctionTitle.BANNISSEMENT;
                    let description = "PROTECTION ANTI-SCAM";
                    let isTesting = false;
                    if (this.isTechnician(auth) && message.content == "$test") {
                        isTesting = true;
                    ...
                    // Send to #alert
                    try {
                        const channelAlert = yield simplediscordbot_1.GuildManager.channel.text.find(this.alertChannel);
                    ...
                    try {
                        const channelRapport = yield simplediscordbot_1.GuildManager.channel.text.find(this.rapportChannel);
                    ...
                    if (isTesting) {
                        channelInfraction = yield simplediscordbot_1.GuildManager.channel.text.find(this.botBrouillonChannel);
                    }
                    else {
                        channelInfraction = yield simplediscordbot_1.GuildManager.channel.text.find(this.infractionChannel);
                    }
                    try {
