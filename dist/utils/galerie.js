...
function galerie(message) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    try {
      ...
      if (
        message.reference &&
        message.reference.type == discord_js_1.MessageReferenceType.Forward
      ) {
        name = "{forwarded}";
        ...
        const ref = message.reference
          ? message.reference.type == discord_js_1.MessageReferenceType.Default
            ? "Answer message"
            : "Forwarded"
          : false;
        embed.fields = [
          ...
          {
            name: "Attachement",
            value: messageData.attachement.toString(),
            inline: true,
          },
        ...
        if (!(yield checkIfMessageStillExist(message))) {
          (0, messages_1.sendMessageToInfoChannel)(
            "This message has already been deleted by another bot"
          );
          return;
        }
        (0, embeds_1.sendEmbedToInfoChannel)(embed);
        const member = yield (0, channels_1.searchClientGuildMember)(
          ((_a = message.member) === null || _a === void 0 ? void 0 : _a.id) ||
            message.author.id
        );
        if (member && (0, members_1.checkIfApplyMember)(member)) {
          ...
          let msg = [
            "Raisons :\n- Veuillez réagir dans les fils prévus.\n- Vous pouvez seulement envoyer des liens / fichiers (images & vidéos) / sondages.",
            "Vous ne pouvez pas écrire dans ce channel.",
          ];
          if (
            message.reference &&
            message.reference.type == discord_js_1.MessageReferenceType.Default
          ...
          let msgRep = yield message.reply(
            (0, embeds_1.returnToSendEmbed)(
              (0, embeds_1.createErrorEmbed)(msg[0], msg[1])
            )
          );
          message.delete();
          yield (0, promises_1.setTimeout)(
            UnitTime_1.Time.second.SEC_12.toMilliseconds()
          );
          msgRep.delete();
          return;
        }
      ...
      if ((yield checkIfMessageStillExist(message)) && !message.hasThread) {
        //hasThread needs to be after the message refresh
        ...
        try {
          yield message.react("<:hdfr_love:1238621674954428498>");
        } catch (e) {
          (0, embeds_1.sendEmbedToInfoChannel)(
            (0, embeds_1.createErrorEmbed)(`${e} :hdfr_love:`)
          );
        }
        try {
          yield message.react("<:hdfr_bonhelldivers:1236794040070963323>");
        } catch (e) {
          (0, embeds_1.sendEmbedToInfoChannel)(
            (0, embeds_1.createErrorEmbed)(`${e} :hdfr_bonhelldivers:`)
          );
        }
        try {
          yield message.react("<:hdfr_xd:1238620802585333791>");
        } catch (e) {
          (0, embeds_1.sendEmbedToInfoChannel)(
            (0, embeds_1.createErrorEmbed)(`${e} :hdfr_xd`)
          );
        }
        try {
          yield message.react("<:hdfr_hitass:1350113011163201607>");
        } catch (e) {
          (0, embeds_1.sendEmbedToInfoChannel)(
            (0, embeds_1.createErrorEmbed)(`${e} :hdfr_hitass`)
          );
        }
      }
    } catch (error) {
      (0,
      embeds_1.sendEmbedToInfoChannel)((0, embeds_1.createErrorEmbed)(`${error} : ${message.url}`));
      console.error(error);
    }
  });
}
function checkIfMessageStillExist(message) {
  return __awaiter(this, void 0, void 0, function* () {
    try {
      yield (0,
      promises_1.setTimeout)(UnitTime_1.Time.second.SEC_01.toMilliseconds());
      yield message.fetch();
      // Success
      return true;
    } catch (e) {
