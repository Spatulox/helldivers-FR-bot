...
const errorRateLimiter = new discord_js_rate_limiter_1.RateLimiter(
  1,
  timeToWait
);
...
function initializeAutomaton() {
  return __awaiter(this, void 0, void 0, function* () {
    try {
      const channel = yield (0, channels_1.searchClientChannel)(
        client_1.client,
        config_json_1.default.counterChannel
      );
      ...
      automatonCounter =
        new AutomatonIntrusionCounter_1.AutomatonIntrusionCounter(
          counterChannel,
          {
            ...
            onHackStart() {
              return __awaiter(this, void 0, void 0, function* () {
                ...
                embed.description = `Une nouvelle intrusion automaton a été crée ici : ${
                  (_a =
                    automatonCounter === null || automatonCounter === void 0
                      ? void 0
                      : automatonCounter.AutomatonMessage) === null ||
                  _a === void 0
                    ? void 0
                    : _a.url
                }`;
      ...
      console.error(error);
      (0,
      embeds_1.sendEmbedToInfoChannel)((0, embeds_1.createErrorEmbed)(`initializeAutomaton : ${error}`));
    }
  });
}
  ...
  return __awaiter(this, void 0, void 0, function* () {
    if (!(yield initializeAutomaton())) {
      (0, messages_1.sendMessageToInfoChannel)(
        "Impossible to initialize the counter channel"
      );
      return;
    }
    yield mutex.lock();
    try {
      const [counterChannel, logChannel] = yield Promise.all([
        (0, channels_1.searchClientChannel)(
          client_1.client,
          config_json_1.default.counterChannel
        ),
        (0, channels_1.searchClientChannel)(
          client_1.client,
          config_json_1.default.helldiverLogChannel
        ),
      ]);
      if (!counterChannel || !counterChannel.isTextBased()) {
        logChannel
          ? (0, embeds_1.sendEmbedErrorMessage)(
              "Counter channel is null T_T",
              logChannel
            )
          : (0, messages_1.sendMessage)("Channel counter is null WELP");
        return;
      }
      const messages = (yield counterChannel.messages.fetch({
        limit: 20,
      })).reverse();
      for (const msg of messages.values()) {
        ...
        if (
          msg.author.bot &&
          msg.author.id === constantes_1.AMIRAL_SUPER_TERRE_ID
        ) {
          // || (msg.author.id === config.clientId && isNaN(n) ) )){
          continue;
        }
        if (!isNaN(n)) {
          // If it's a number
          if (EXPECTED == 0 || n == EXPECTED || msg.author.bot) {
            // EXPECTED is 0 => Initial state // n = EXPECTED => Coherent number // msg.author.bot => Bot message AND is number
            COUNT = EXPECTED = n;
            EXPECTED++;
          } else {
            try {
...
function incrementCounter(message) {
  return __awaiter(this, void 0, void 0, function* () {
    const avoid = [
      constantes_1.AMIRAL_SUPER_TERRE_ID,
      config_json_1.default.clientId,
    ]; // The Automaton Webhook ID still can pass since it's not the current bot
    ...
    try {
      ...
      if (!message.author.bot && automatonCounter.isHacked) {
        // If non bot and hacked
      ...
      if (number === EXPECTED && !automatonCounter.isHacked) {
        ...
        if ((Math.random() <= 0.1 && DAY) || (Math.random() <= 0.04 && NIGHT)) {
          ...
          try {
            ...
            (0, embeds_1.sendEmbedToInfoChannel)(
              (0, embeds_1.createErrorEmbed)(
                `1% proba counter Automaton Intrusion ${error}`
              )
            );
...
function handleNonNumeric(message) {
  return __awaiter(this, void 0, void 0, function* () {
    var _a;
    try {
      const member = yield (_a = message.guild) === null || _a === void 0
        ? void 0
        : _a.members.fetch(message.author.id);
      if (member && (0, members_1.checkIfApplyMember)(member)) {
        yield replyAndDeleteReply(
          message,
          `Le message doit commencer par un nombre :\n> - 12 exemple\n-# Ceci n'est pas compté comme une erreur`
        );
      }
      ...
      console.error(error);
      (0,
      embeds_1.sendEmbedToInfoChannel)((0, embeds_1.createErrorEmbed)(`handleNonNumeric error : ${error}`));
    }
  });
}
function handleMismatch(message, number) {
  return __awaiter(this, void 0, void 0, function* () {
    try {
      ...
      let to = false,
        msg = ":warning: Ce n'est pas le bon nombre... :eyes:";
      if (diff > 10) {
      ...
      console.error(error);
      (0,
      embeds_1.sendEmbedToInfoChannel)((0, embeds_1.createErrorEmbed)(` handleMismatch error ${error}`));
    }
  });
}
  ...
  return __awaiter(this, arguments, void 0, function* (message, force = false) {
    ...
    try {
      ...
      if (
        ...
        (force || errorRateLimiter.take(message.author.id))
      ) {
        try {
          yield (_b = message.member) === null || _b === void 0
            ? void 0
            : _b.timeout(timeToWait);
          return true;
        } catch (err) {
...
function notifyAdmin(message, number, to) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const adminChannel = yield (0, channels_1.searchClientChannel)(
      client_1.client,
      config_json_1.default.adminChannel
    );
    const logChannel = yield (0, channels_1.searchClientChannel)(
      client_1.client,
      config_json_1.default.helldiverLogChannel
    );
    if (adminChannel) (0, embeds_1.sendEmbed)(embed, adminChannel);
    if (logChannel) (0, embeds_1.sendEmbed)(embed, logChannel);
    if (!adminChannel && !logChannel) {
      (0, messages_1.sendMessageToAdminChannel)(
        (_a = embed.description) !== null && _a !== void 0 ? _a : ""
      );
      (0, messages_1.sendMessageToInfoChannel)(
        (_b = embed.description) !== null && _b !== void 0 ? _b : ""
      );
    }
  });
}
  ...
  return __awaiter(this, void 0, void 0, function* () {
    try {
      const reply = yield message.reply(
        (0, embeds_1.returnToSendEmbed)((0, embeds_1.createErrorEmbed)(msg))
      );
      setTimeout(() => {
        reply.delete().catch(() => {});
      }, UnitTime_1.Time.second.SEC_10.toMilliseconds());
      ...
      console.error(error);
      (0,
      embeds_1.sendEmbedToInfoChannel)((0, embeds_1.createErrorEmbed)(` replyAndDeleteReply error ${error}`));
    }
  });
}
