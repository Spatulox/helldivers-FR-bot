...
let oneArrowPerPersonLimiter = new discord_js_rate_limiter_1.RateLimiter(
  1,
  UnitTime_1.Time.day.DAY_01.toMilliseconds()
);
const left = { unicode: "⬅️", custom: "<:hdfr_gauche:1221201626816053408>" };
const right = { unicode: "➡️", custom: "<:hdfr_droite:1221201658151960667>" };
const up = { unicode: "⬆️", custom: "<:hdfr_haut:1221201670479024188>" };
const down = { unicode: "⬇️", custom: "<:hdfr_bas:1221201613226512505>" };
class AutomatonIntrusion {
  constructor(targetChannel, options) {
    ...
    this.rateArrowTimeLimiter = new discord_js_rate_limiter_1.RateLimiter(
      1,
  ...
  get currentStratagemLength() {
    ...
    return this.choosenStratagem
      ? (_b =
          (_a = this.stratagems[this.choosenStratagem]) === null ||
          _a === void 0
            ? void 0
  ...
  get currentStratagemExpectedEmoji() {
    var _a;
    if (!this.choosenStratagem) return;
    const stratagem =
      (_a = this.stratagems) === null || _a === void 0
        ? void 0
        : _a[this.choosenStratagem];
    return stratagem &&
  ...
  handleStratagemInput(message_1) {
    return __awaiter(
      ...
      arguments,
      void 0,
      function* (message, oneArrowPerPerson = false, canReset = false) {
        var _a, _b, _c, _d, _e;
        ...
        try {
          const member = yield (0, channels_1.searchClientGuildMember)(
            message.author.id
          );
          isTechnician =
            member && (0, members_1.checkIfMemberIsTechnician)(member); // || member?.id === "556461959042564098" // Debug statement
        } catch (error) {
          isTechnician = false;
        }
        if (!this._authorizedEmoji.includes(userInput)) {
          ...
          oneArrowPerPerson &&
          oneArrowPerPersonLimiter.take(message.author.id) &&
          this._authorizedEmoji.includes(userInput)
        ) {
          if (!isTechnician) {
            this.callbacks.onWrongStratagemStep &&
              (yield this.callbacks.onWrongStratagemStep(
                message,
                `Vous ne pouvez pas jouer plusieurs fois, sauf si le code est réinitialisé`,
                true
              ));
          ...
          if (!isTechnician) {
            this.callbacks.onWrongStratagemStep &&
              (yield this.callbacks.onWrongStratagemStep(
                message,
                `Veuillez attendre 5 minutes entre chaque envoi de flèche`,
                true
              ));
            message.deletable && (yield message.delete());
            return false;
        ...
        if (!this.isInHackedState || !this._choosenStratagem) return false;
        if (expectedEmoji && Object.values(expectedEmoji).includes(userInput)) {
          if (isTechnician && isTechnicianBypass) {
            try {
              const embed = (0, embeds_1.createEmbed)();
              embed.title = `Technician Bypass ${
                (_a = this.webhookMember[this._choosenMember || "NULL"]) ===
                  null || _a === void 0
                  ? void 0
                  : _a[0]
              }`;
              embed.description = `<@${
                message.author.id
              }> utilisé son droit de bypass pour envoyer une flèche dans le mini-jeu Automaton Intrusion ${
                message.url
              } : ${
                (_b = this._AutomatonMessage) === null || _b === void 0
                  ? void 0
                  : _b.url
              }`;
              (0, embeds_1.sendEmbedToInfoChannel)(embed);
              (0, embeds_1.sendEmbedToAdminChannel)(embed);
              yield (0, messages_1.replyAndDeleteReply)(
                message,
                `Vous avez utilisé votre droit de bypass pour envoyer une flèche dans le mini-jeu Automaton Intrusion`
              );
            } catch (error) {
              console.error(error);
              (0, embeds_1.sendEmbedToInfoChannel)(
                (0, embeds_1.createErrorEmbed)(
                  `handleStratagemInput AdminBypass : ${error}`
                )
              );
            }
          }
          ...
          if (
            ...
            yield (_c = this._AutomatonMessage) === null || _c === void 0
              ? void 0
              : _c.react("💥");
            if (oneArrowPerPerson) {
              oneArrowPerPersonLimiter =
                new discord_js_rate_limiter_1.RateLimiter(
                  1,
                  UnitTime_1.Time.day.DAY_01.toMilliseconds()
                );
            }
          ...
          if (!expectedEmoji) {
            ...
            (0, embeds_1.sendEmbedToInfoChannel)(
              (0, embeds_1.createErrorEmbed)("No Expected Emoji")
            );
            return false;
          }
          ...
          if (emojiCount >= 2) {
            this.callbacks.onWrongStratagemStep &&
              (yield this.callbacks.onWrongStratagemStep(
                ...
                `Une étape à la fois! ${
                  ...
                  ((_d = this.stratagems[this._choosenStratagem]) === null ||
                  _d === void 0
                    ? void 0
                    : _d
            ...
            this.callbacks.onWrongStratagemStep &&
              (yield this.callbacks.onWrongStratagemStep(
                message,
                `Mauvaise étape de code ${
                  canReset
                    ? ": Réinitialisation du stratagème, il faut reprendre du début"
                    : ""
                }\nCode Stratagème : \n${
                  ((_e = this.stratagems[this._choosenStratagem]) === null ||
                  _e === void 0
                    ? void 0
                    : _e
                        .map((emoji) => emoji.custom)
  ...
  endHack(success) {
    return __awaiter(this, void 0, void 0, function* () {
      var _a;
      try {
        yield (_a = this._thread) === null || _a === void 0
          ? void 0
          : _a.delete();
        ...
        console.error(error);
        (0,
        embeds_1.sendEmbedToInfoChannel)((0, embeds_1.createErrorEmbed)(`endHack : ${error}`));
      }
    });
  }
  ...
  countAuthorizedEmojisInMessage(content) {
    ...
    for (const emoji of this._authorizedEmoji)
      for (
        let idx = content.indexOf(emoji);
        idx !== -1;
        idx = content.indexOf(emoji, idx + emoji.length)
      )
        count++;
    return count;
  }
  sendWebhook(content, channel_id) {
    return __awaiter(this, void 0, void 0, function* () {
      const counterChannel = yield (0, channels_1.searchClientChannel)(
        client_1.client,
        channel_id ? channel_id : config_json_1.default.counterChannel
      );
      if (!counterChannel) {
        return null;
      }
  ...
  static cleanOldIntrusion(client) {
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        if (!guild) {
          console.error(
            `Guild with ID ${constantes_1.TARGET_GUILD_ID} not found or not in cache.`
          );
          return;
        }
            ...
            for (const thread of fetched.threads.values()) {
              if (thread.name === "Intrusion Automaton") {
                try {
                  ...
                  console.error(
                    `Error fetching starter message for thread ${thread.id} : `,
                    error
                  );
                } finally {
                  yield thread.delete(
                    "Nettoyage automatique des threads d'intrusion Automaton"
                  );
                }
              }
            }
        ...
        (0, log_1.log)("Thread check finished");
        this.mutex.unlock();
      } catch (error) {
        console.error(error);
        (0,
        embeds_1.sendEmbedToInfoChannel)((0, embeds_1.createErrorEmbed)(`cleanOldIntrusion : ${error}`));
      }
    });
  }
}
