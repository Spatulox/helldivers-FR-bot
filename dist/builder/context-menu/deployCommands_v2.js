        ...
        client_1.client.once("ready", () => __awaiter(this, void 0, void 0, function* () {
                        ...
                        if (command.guildID && command.guildID.length > 0) {
                            for (const guildId of command.guildID) {
                                ...
                                try {
                                    ...
                                    (0, log_1.log)(`SUCCÈS : Menu Contextuel "${command.name}" déployée sur la guilde ${guildId}`);
                                }
                                catch (err) {
                            ...
                            (0, log_1.log)(`AJOUT : Menu contextuel "${command.name}" ajouté à la file d'attente`);
            ...
            if (commandArray.length > 0) {
                try {
                    ...
                    (0, log_1.log)(`SUCCÈS : ${numberCommandDeployed}/${fileLength} menus contextuels globaux déployés`);
                }
                catch (err) {
                    (0, log_1.log)(`ERREUR CRITIQUE : Déploiement des menus contextuels globaux : ${err.message}`);
                }
            }
            process.exit();
        }));
